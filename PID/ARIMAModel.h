...
public:
	...
	std::vector<double> preFirDiff(std::vector<double> preData) {
		...
		for (unsigned int i = 0; i < preData.size() - 1; i++) {
	...
	std::vector<double> preSeasonDiff(std::vector<double> preData) {
		...
		for (unsigned int i = 0; i < preData.size() - 7; i++) {
	...
	std::vector<double> preDealDiff(unsigned int period) {
	...
	std::vector<int> getARIMAModel(int period, std::vector<std::vector<int> > notModel, bool needNot) {
		...
		for (int i = 0; i < cnt; ++i)
		{
			...
			if (needNot)
			{
				for (unsigned int k = 0; k < notModel.size(); ++k)
				{
	...
	double aftDeal(double predictValue, unsigned int period) {
		...
		case 0:
			return predictValue;
		case 1:
			return (predictValue + dataArray[dataArray.size() - 1]);
		case 2:
		default:
			return (predictValue + dataArray[dataArray.size() - 7]);
		}
	}
	...
	double predictValue(int p, int q, int period) {
		std::vector<double> data(this->preDealDiff(period));
		int n = data.size();
		double predict = 0;
	...
	double forecastValue(int p, int q, int period, int j) {
		for (int i = 0; i < j; i++)
		{
			double predictDiff = (this->predictValue(p, q, period));

			dataArray.push_back(this->aftDeal(predictDiff, period));
		}
		double forecast = dataArray.back();
	...
	double arma_forecast(int modelCnt, int period, int step)
	{
		int cnt = 0;
		std::vector<std::vector<int> > list;
		std::vector<double> tmpPredict(modelCnt);
		for (int k = 0; k < modelCnt; ++k)			//控制通过多少组参数进行计算最终的结果
		{
			std::vector<int> bestModel = this->getARIMAModel(period, list, (k == 0) ? false : true);
			//std::cout<<bestModel.size()<<std::endl;

			if (bestModel.size() == 0)
			{
				tmpPredict[k] = dataArray[dataArray.size() - period];
				cnt++;
				break;
			}
			else
			{
				...
				tmpPredict[k] = this->forecastValue(bestModel[0], bestModel[1], period, step);
